...
class PDOMCPPClassSpecialization extends PDOMCPPSpecialization implements
	...
	public int getKey() throws DOMException {
		IBinding binding = getSpecializedBinding();
		if (!(binding instanceof ICPPClassType)) {
			CCorePlugin.log(binding.getName() + " is not a cpp class type. (" + binding.toString() + ")"); //$NON-NLS-1$ //$NON-NLS-2$
			return 0;
		}
		return ((ICPPClassType)binding).getKey();
	}
